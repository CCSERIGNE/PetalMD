...
const CardPokemon: React.FC<{ name: string }> = ({ name }) => {
  ...
  useEffect(() => {
    api.get(`/pokemon/${name}`).then(response => {
      ...
      // Tout Pokémon avec deux classes ou plus et qui a le type "normal" comme première classe,
      // Ce codicinal vous oblige à prendre le deuxième cours, la raison de ce flux est le meilleur style de la mise en page de l'application
      ...
      setPokemon({
        ...
        type: types.map((pokemonType: any) => {
          // Reconnaît la variable comme une clé pour les types pokemon et les tableaux colours.type
